...
def good_subsequences(word):
 ...
 'bc', 'bca', 'c', 'ca', 'cab', 'cb', 'cba']
    ...
    result_list.append('')
    s = ''
    re(s,0, word, result_list)
    result_list.sort()
    print(result_list)
    
# Possibly define another function
def re(s, position, word, record_list):
    if s not in record_list:
        record_list.append(s)
    if position < len(word):
        if word[position] not in s:
            re(s+word[position], position+1, word, record_list)
        re(s, position+1, word, record_list)
