...
def is_word_word_ladder(word_1, word_2, candidate_ladder):
    ...
    result = (word_1 in lexicon and word_2 in lexicon) and \
             (word_1 is candidate_ladder[0] and word_2 is candidate_ladder[-1])
    if result:
        if len(candidate_ladder) > 2:            
            sequence = -1
            for i in range(1,len(candidate_ladder),1):
                if candidate_ladder[i] in closest_words[candidate_ladder[i-1]]:
                    can_list1 = list(candidate_ladder[i-1])
                    can_list2 = list(candidate_ladder[i])
                    for j in range(len(can_list1)):
                        if can_list1[j] != can_list2[j]:
                            if sequence != j:
                                sequence = j
                            else:
                                result = False
