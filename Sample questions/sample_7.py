...
def find_word(filename, word):
    ...
    with open(filename) as file:
        ...
        for line in file:
            if not line.isspace():
                letter_list = list(line.replace(" ",""))
                grid.append(letter_list)
...
def find_word_horizontally(grid, word):
    ...
    for i in range(len(grid)):
        for j in range(len(grid[i])):
            if grid[i][j] is word_list[0]:
                number = 0
                while j+number < len(grid[0])-1 and \
                      grid[i][j+number] is word_list[number]:
                    number += 1
...
def find_word_vertically(grid, word):
    ...
    for i in range(len(grid)):
        for j in range(len(grid[i])):
            if grid[i][j] is word_list[0]:
                number = 0
                while i+number < len(grid) - 1 and \
                      grid[i+number][j] is word_list[number]:
                    number += 1
                    if number == len(word_list):
                        return (i+1,j+1)
    return 0
    ...
    for i in range(len(grid)):
        for j in range(len(grid[i])):
            if grid[i][j] is word_list[0]:
                number = 0
                while j+number < len(grid[0])-1 and \
                      i+number < len(grid)-1 and \
                      grid[i+number][j+number] is word_list[number]:
                    number += 1
                    if number == len(word_list):
                        return (i+1,j+1)
    return 0
